<!-- CARDS go here -->
<section class="resource_3_card_image_and_text resource_5or6 pad-section pad-bottom-xl">
	...
	<?php endif ?>
	<!--  END -->
	<div class="grid-container green-bg-box-before relative margin-top-2">
		<!-- Check for layout type -->
		<?php

			if ( count(get_sub_field('cards') ) == 6) {
					$layout = 'large-up-3';
					$weighted = '';
				}
			else {
							...
							<?php if ($topics): ?>
								<div class="dark-slate flexo-bold topic">
									...
									<?php foreach ($topics as $key => $value): ?>
										<?php 
											$space = ($i == sizeOf($topics)) ? ' ' : ', ';
											echo $value->name.$space;
										 ?>
										<?php ++$i ?>
									<?php endforeach ?>
								</div>
