...
class AutoUpdater_WP_Admin
{
    ...
    protected        $menu_slug = 'smart-plugin-manager';
    ...
    {
        $this->menu_slug = get_option('config_url', 'smart-plugin-manager');
        if (is_admin())
        {
    ...
    public function displayConfigurationPage()
    {
        ...
        if ($autoupdater_available)
        {
            ...
            $update_cms                     = AutoUpdater_Config::get('update_cms', 0);
            ...
            $template_inactive = $template_active = $autoupdater_enabled ?
