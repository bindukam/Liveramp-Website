...
class AutoUpdater_Authentication
{
    ...
    public function validate($payload)
    {
        ...
        {
            $url = 'http' . (is_ssl() ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
            AutoUpdater_Log::error(sprintf('Invalid signature. Received %s request to %s', strtoupper($method), $url));
