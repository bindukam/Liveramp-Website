...
{
    protected static $host     = 'autoupdater.wpengine.com';
    ...
    protected function saveAutoUpdaterConfig($data)
    {
        ...
        if ($this->getOption('autoupdater_available'))
        {
            ...
            if (array_key_exists('maintenance_mode', $data)) {
                $settings['maintenance_mode'] = (int) $data['maintenance_mode'];
                if ((int) $this->getOption('maintenance_mode') !== $settings['maintenance_mode']) {
                  $changed = true;
                }
            }
